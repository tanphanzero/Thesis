...
ros_node::ros_node(std::shared_ptr<driver> driver, int argc, char **argv)
{
    ...
    ros_node::m_publisher_data_raw = ros_node::m_node->advertise<sensor_msgs::Imu>("imu/data_raw", 1);
    //ros_node::m_publisher_data_raw2 = ros_node::m_node->advertise<sensor_msgs::Imu>("imu/data_raw2", 1);
    ros_node::m_publisher_mag = ros_node::m_node->advertise<sensor_msgs::MagneticField>("imu/mag", 1);
...
void ros_node::data_callback(driver::data data)
{
    ...
    //imu2.header.frame_id = "base_link";
    ...
    //imu2.linear_acceleration.x = message_accel.x;
    //imu2.linear_acceleration.y = message_accel.y;
    //imu2.linear_acceleration.z = message_accel.z;
    // Apply calibration.
    ...
    //imu2.angular_velocity.x = message_gyro.x;
    //imu2.angular_velocity.y = message_gyro.y;
    //imu2.angular_velocity.z = message_gyro.z;
    //ros_node::m_publisher_data_raw2.publish(imu2);
    ...
    // Set message containing raw IMU data, including angular velocities and linear accelerations
    imu.angular_velocity.x = message_gyro.x;
    imu.angular_velocity.y = message_gyro.y;
    imu.angular_velocity.z = message_gyro.z;

    imu.linear_acceleration.x = message_accel.x;
    imu.linear_acceleration.y = message_accel.y;
    imu.linear_acceleration.z = message_accel.z;

    // Publish message.
    //ros_node::m_publisher_gyroscope.publish(message_gyro);
    ros_node::m_publisher_data_raw.publish(imu);
    ...
    if(std::isnan(data.magneto_x) == false)
    {
        ...
        mag.magnetic_field.x = message_mag.x;
        mag.magnetic_field.y = message_mag.y;
        mag.magnetic_field.z = message_mag.z;

        // Publish message.
        //ros_node::m_publisher_magnetometer.publish(message_mag);
        ros_node::m_publisher_mag.publish(mag);
    }
